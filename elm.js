...
Elm.Timer.make = function (_elm) {
   ...
   var _op = {};
   var clock = A2($Signal.map,$Basics.round,A3($Signal.foldp,F2(function (x,y) {    return x + y;}),0,$Time.every($Time.second)));
   var addLeadingZero = function (num) {    return _U.cmp(num,10) < 0 ? A2($Basics._op["++"],"0",$Basics.toString(num)) : $Basics.toString(num);};
   var update = F2(function (action,model) {
      var _p0 = action;
      if (_p0.ctor === "Tick") {
            return model + 1;
         } else {
            return _U.cmp(model,0) > 0 ? model - 1 : model;
         }
   });
   var Tock = {ctor: "Tock"};
   var countdown = A2($Signal.map,$Basics.always(Tock),clock);
   var Tick = {ctor: "Tick"};
   var tick = A2($Signal.map,$Basics.always(Tick),clock);
   var view = function (model) {
      var minutes = function () {    var mins = A2($Basics._op["%"],model / 60 | 0,60);return addLeadingZero(mins);}();
      var seconds = function () {    var secs = A2($Basics._op["%"],model,60);return addLeadingZero(secs);}();
      return A2($Basics._op["++"],minutes,A2($Basics._op["++"],":",seconds));
   };
   var init$ = function (model) {    return model;};
   var init = 0;
   return _elm.Timer.values = {_op: _op
                              ,init: init
                              ,init$: init$
                              ,view: view
                              ,update: update
...
Elm.Main.make = function (_elm) {
   ...
   var view = function (_p0) {
      ...
      _U.list([A2($Html.div,
              _U.list([]),
              _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"A regular timer: ",$Timer.view(_p1.timer1)))]))
                      ,A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"Starts at 12:00: ",$Timer.view(_p1.timer2)))]))]))
              ,$Html.text(A2($Basics._op["++"],"Model: ",$Basics.toString(_p1)))]));
   };
   var update = F2(function (action,model) {
      var _p2 = A2($Debug.log,"Main",action);
      if (_p2.ctor === "Timer1") {
            return _U.update(model,{timer1: A2($Timer.update,_p2._0,model.timer1)});
         } else {
            return _U.update(model,{timer2: A2($Timer.update,_p2._0,model.timer2)});
         }
   });
   var Timer2 = function (a) {    return {ctor: "Timer2",_0: a};};
   var Timer1 = function (a) {    return {ctor: "Timer1",_0: a};};
   var input = A2($Signal.merge,A2($Signal.map,Timer1,$Timer.tick),A2($Signal.map,Timer2,$Timer.tick));
   var initialModel = {timer1: $Timer.init,timer2: function (_p3) {    return $Timer.init$($Basics.round(_p3));}($Time.inSeconds($Time.minute) * 12)};
   var model = A3($Signal.foldp,update,initialModel,input);
   var main = A2($Signal.map,view,model);
   var Model = F2(function (a,b) {    return {timer1: a,timer2: b};});
